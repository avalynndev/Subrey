...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    if (data.Paused === true) {
      embed.setColor("Red").setDescription("This giveaway is paused");
      return interaction.reply({ embeds: [embed], ephemeral: true });
    }

    if (data.Ended === true) {
      embed.setColor("Red").setDescription("This giveaway has ended");
      return interaction.reply({ embeds: [embed], ephemeral: true });
    }
    ...
    await DB.findOneAndUpdate(
      ...
      embed.setColor("Green").setDescription("You have joined the giveaway");
      return interaction.reply({ embeds: [embed], ephemeral: true });
    });
  },
};
